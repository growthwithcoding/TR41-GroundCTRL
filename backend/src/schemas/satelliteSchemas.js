...
const orbitSchema = z
	.object({
		...
		eccentricity: z
			...
			.max(
				0.99,
				"Eccentricity must be between 0 and 0.99 (parabolic/hyperbolic orbits not supported)",
			)
			.default(0.0001)
			.describe(
				"Orbital eccentricity (0 = circular, 0.99 = highly elliptical)",
			),
	...
	.object({
		query: z
			.object({
				...
				status: statusEnum.optional().describe("Filter by satellite status"),
				isPublic: z
					.string()
					.optional()
