			...
			.object({
				...
				type: z
					.enum(CERTIFICATE_TYPES, {
						errorMap: () => ({
							message: `Type must be one of: ${CERTIFICATE_TYPES.join(", ")}`,
						}),
					})
				...
				isPublic: z
					.boolean()
					.default(true)
					.describe("Whether certificate can be publicly viewed"),
			})
			.strict(),
	})
	.strict();
...
const patchCertificateSchema = z
	.object({
		params: z
			.object({
				id: z
	...
	.object({
		query: z
			.object({
				...
				sortBy: z
					.enum(["completionDate", "createdAt", "overallScore", "type"])
					.optional()
				...
				tier: z
					.enum(PERFORMANCE_TIERS)
					.optional()
					.describe("Filter by performance tier"),
				isPublic: z
					.string()
					.optional()
					.transform((val) => val === "true")
					.describe("Filter by public visibility"),
			})
			.strict(),
	})
	.strict();
...
const getCertificateBySessionSchema = z
	.object({
		params: z
			.object({
				sessionId: z
	...
	.object({
		...
		body: z
			.object({
				isPublic: z
					.boolean()
					.describe("Whether certificate is publicly viewable"),
			})
			.strict(),
	})
	.strict();
