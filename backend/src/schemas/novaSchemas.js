	...
	.object({
		body: z
			.object({
				...
				hint_type: HintType.optional().describe(
					"Type of hint provided (for assistant messages)",
				),
	...
	.object({
		...
		body: z
			.object({
				...
				hint_type: HintType.optional().describe("Type of hint provided"),
	...
	.object({
		...
		query: z
			.object({
				...
				role: MessageRole.optional().describe("Filter by message role"),
			})
			.strict(),
	})
	.strict();
		...
		body: z
			.object({
				...
				content: z
					...
					.refine(
						(val) =>
							!/<script\b[\s\S]*?>/i.test(val) &&
							!/<\/script\b[\s\S]*?>/i.test(val),
						{ message: "Scripts not allowed in content" },
					)
					.refine(
						(val) =>
							!/<iframe\b[\s\S]*?>/i.test(val) &&
							!/<\/iframe\b[\s\S]*?>/i.test(val),
...
const updateSessionHintsSchema = z
	.object({
		params: z
			.object({
...
const novaSuggestionSchema = z.object({
	id: z
		...
		.describe(
			'Unique suggestion identifier (e.g., "modules", "recommend", "hint")',
		),
		...
		body: z
			.object({
				content: z
					...
					.refine(
						(val) =>
							!/<\s*script\b[^>]*>/i.test(val) &&
							!/<\s*\/\s*script\b[^>]*>/i.test(val),
						{ message: "Scripts not allowed in content" },
					)
					.refine(
						(val) =>
							!/<\s*iframe\b[^>]*>/i.test(val) &&
							!/<\s*\/\s*iframe\b[^>]*>/i.test(val),
						{ message: "Iframes not allowed in content" },
					)
					.refine((val) => !/on\w+\s*=/i.test(val), {
						message: "Event handlers not allowed in content",
				...
				conversationId: z
					...
					.optional()
					.describe(
						"Optional conversation ID for multi-turn chat (generated on first request)",
					),
				...
				sessionId: z
					...
					.optional()
					.describe(
						"Optional session ID for authenticated users in training mode",
					),
