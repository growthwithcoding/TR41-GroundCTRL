...
const consoleLayoutSchema = z
	.object({
		panels: z
			.array(z.string())
			.optional()
			.describe("Which panels appear in mission console"),
		widgets: z
			.array(z.string())
			.optional()
			.describe("Which widgets appear in mission console"),
	})
	.strict()
	.optional();
		...
		body: z
			.object({
				...
				code: z
					...
					.regex(
						/^[A-Z0-9_]+$/,
						"Scenario code must be uppercase alphanumeric with underscores",
					)
				...
				tier: tierEnum.describe(
					"Pilot tier required (ROOKIE_PILOT, MISSION_SPECIALIST, MISSION_COMMANDER)",
				),
				...
				consoleLayout: consoleLayoutSchema.describe(
					"Which panels/widgets appear in mission console",
				),
	...
	.object({
		...
		body: z
			.object({
				...
				description:
					createScenarioSchema.shape.body.shape.description.optional(),
				difficulty: difficultyEnum.optional(),
				tier: tierEnum.optional(),
				type: typeEnum.optional(),
				estimatedDurationMinutes:
					createScenarioSchema.shape.body.shape.estimatedDurationMinutes.optional(),
	...
	.object({
		query: z
			.object({
				...
				tier: tierEnum.optional().describe("Filter by pilot tier"),

				type: typeEnum
					.optional()
					.describe("Filter by scenario type (GUIDED or SANDBOX)"),

				status: statusEnum.optional().describe("Filter by publishing status"),
				...
				satellite_id: z.string().optional().describe("Filter by satellite ID"),
			})
			.strict(),
	})
	.strict();
