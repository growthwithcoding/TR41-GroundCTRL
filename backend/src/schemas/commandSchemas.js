...
const VALID_COMMAND_NAMES = [
	...
	// Anomaly Resolution Commands (Anomaly System Enhancement)
	"RESET_SOLAR",
	"RESET_POWER_BUS",
	"DESATURATE_WHEELS",
	"SWITCH_ATTITUDE_MODE",
	"CYCLE_THERMAL_LOUVERS",
	"REPOINT_ANTENNA",
	"RESET_TRANSPONDER",
	"SWITCH_FREQUENCY",
	"CYCLE_THRUSTER_VALVES",
	"PRESSURIZE_FUEL_TANK",
	"CALIBRATE_PAYLOAD",
	"FORMAT_STORAGE",
];
	...
	.object({
		body: z
			.object({
				...
				commandName: z
					.enum(VALID_COMMAND_NAMES, {
						errorMap: () => ({
							message: `commandName must be one of: ${VALID_COMMAND_NAMES.join(", ")}`,
						}),
					})
				...
				commandPayload: z
					...
					.default({})
					.describe(
						"Command parameters as JSON (structure varies by commandName)",
					),
				...
				resultStatus: z
					.enum(RESULT_STATUS, {
						errorMap: () => ({
							message: `resultStatus must be one of: ${RESULT_STATUS.join(", ")}`,
						}),
					})
					.optional()
		...
		query: z
			.object({
				...
				scenario_step_id: z
					.string()
					.optional()
					.describe("Filter by scenario step ID"),
		...
		body: z
			.object({
				...
				command_name: z
					.enum(VALID_COMMAND_NAMES, {
						errorMap: () => ({
							message: `command_name must be one of: ${VALID_COMMAND_NAMES.join(", ")}`,
						}),
					})
					.describe("Command type from valid command registry"),
