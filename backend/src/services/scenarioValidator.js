...
class ScenarioValidator {
	...
	validateOrbitalManeuver(config, state) {
		...
		if (config.targetApoapsis) {
			const inRange =
				apoapsis >= config.targetApoapsis.min &&
		...
		if (config.targetPeriapsis) {
			const inRange =
				periapsis >= config.targetPeriapsis.min &&
		...
		if (config.targetAltitude) {
			...
			const inRange =
				altitude >= config.targetAltitude.min &&
		...
		if (config.fuelRemaining) {
			const fuelKg =
				state.propellantMass_kg || state.resources?.propellantMass_kg || 0;
	...
	validateMissionCompletion(config, state) {
		...
		if (config.requiredStepsCompleted) {
			...
			const allCompleted = config.requiredStepsCompleted.every((step) =>
	...
	validateCommandExecuted(config, state) {
		...
		const matchingCommand = commandHistory.find((cmd) => {
			...
			if (parameters) {
				const cmdParams =
					cmd.params || cmd.parameters || cmd.commandPayload || {};
		...
		const commandSucceeded =
			matchingCommand?.resultStatus === "OK" ||
	...
	validateCommandSequence(config, state) {
		...
		for (const cmdName of commands) {
			const cmdIndex = commandHistory.findIndex(
				(cmd, idx) =>
					idx > lastIndex &&
