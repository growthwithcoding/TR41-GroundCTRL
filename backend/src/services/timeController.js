...
class TimeController {
	...
	setTimeScale(sessionId, newScale, reason = "manual") {
		...
		if (!timeState) {
			logger.warn("Cannot set time scale - session not initialized", {
				sessionId,
			});
			return null;
		}
	...
	setCriticalOperation(sessionId, isActive, operationType = "unknown") {
		...
		if (!timeState) {
			logger.warn("Cannot set critical operation - session not initialized", {
				sessionId,
			});
			return;
		}
	...
	createTimePrompt(sessionId, reason, suggestedScale, estimatedWaitTime) {
		...
		if (!timeState) {
			logger.warn("Cannot create prompt - session not initialized", {
				sessionId,
			});
			return null;
		}
	...
	respondToPrompt(sessionId, promptId, accepted, customScale = null) {
		...
		if (!timeState) {
			logger.warn("Cannot respond to prompt - session not initialized", {
				sessionId,
			});
			return;
		}

		const promptIndex = timeState.pendingPrompts.findIndex(
			(p) => p.id === promptId,
		);
	...
	recommendTimeScale(step, sessionState) {
		...
		if (step.completionCondition?.type === "time_elapsed") {
			...
			if (requiredSeconds > 300) {
				// More than 5 minutes
				recommendedScale = this.TIME_SCALES.FAST_60X;
				reason = "long_wait";
			} else if (requiredSeconds > 120) {
				// More than 2 minutes
