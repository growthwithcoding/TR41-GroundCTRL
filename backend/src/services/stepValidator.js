...
class StepValidator {
	...
	validateStepCompletion(step, sessionState, commandHistory) {
		...
		if (!validationMethod) {
			...
			return {
				isComplete: false,
				reason: `Unknown validation type: ${condition.type}`,
			};
		}
	...
	validateCommandExecution(condition, _sessionState, commandHistory) {
		...
		const executedCommand = commandHistory.find(
			(cmd) => cmd.commandName === requiredCommand && cmd.resultStatus === "OK",
		);
	...
	checkStrictSequence(requiredCommands, commandHistory) {
		...
		for (const cmd of commandHistory) {
			if (
				cmd.commandName === requiredCommands[requiredIdx] &&
				cmd.resultStatus === "OK"
			) {
				requiredIdx++;
	...
	checkFlexibleSequence(requiredCommands, commandHistory) {
		...
		const missing = requiredCommands.filter(
			(cmd) => !completedCommands.includes(cmd),
		);
	...
	validateSubsystemStatus(condition, sessionState, _commandHistory) {
		...
		if (!subsystem || !requiredStatus) {
			return {
				isComplete: false,
				reason: "Incomplete subsystem status condition",
			};
		}
