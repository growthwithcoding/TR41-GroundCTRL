...
class CommandHandler {
  ...
  async executeBurn(satellite, burnParams, currentStep = {}) {
    ...
    const validation = this.validateBurnCommand(satellite, burnParams, currentStep);
    ...
    const deltaVRemaining_ms = thruster.Isp_sec * CONSTANTS.G0 * 
      Math.log(m0_remaining / m1_min);
  ...
  applyBurnToOrbit(elements, direction, deltaV_ms) {
    ...
    case 'radial_out':
      ...
      a_new = a;
      e_new = e + (direction === 'radial_out' ? 0.01 : -0.01) * (deltaV_kmps / 0.1);
   ...
   */
  calculateHohmannBurnTiming(currentElements, targetAltitude, phase = 'raise_apoapsis') {
    ...
    if (phase === 'raise_apoapsis') {
      ...
      const period = 2 * Math.PI * Math.sqrt(Math.pow(a, 3) / CONSTANTS.MU_EARTH);
