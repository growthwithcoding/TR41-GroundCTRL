...
function initializeFirebase() {
	...
	if (process.env.FIREBASE_API_KEY) {
		process.env.FIREBASE_API_KEY = process.env.FIREBASE_API_KEY.replace(
			/^"|"$/g,
			"",
		);
	}
	...
	try {
		...
		// Test mode without emulators: Skip Firebase initialization entirely
			...
			return {
				...admin,
				initializeApp: () => {
					// No-op in test mode - Firebase not initialized
				},
				firestore: () => ({
					collection: () => ({
						doc: () => ({
				...
				auth: () => ({
					verifyIdToken: () =>
						Promise.reject(
							new Error("Firebase auth not available in test mode"),
						),
					createCustomToken: () =>
						Promise.reject(
							new Error("Firebase auth not available in test mode"),
		...
		// Firebase App Hosting / Cloud Run provides credentials automatically
			...
			logger.info(
				"Using Application Default Credentials for Firebase Admin (production)",
				{
		...
		// Development mode: Use explicit service account credentials
			...
			if (!process.env.FIREBASE_CLIENT_EMAIL) {
				throw new Error(
					"FIREBASE_CLIENT_EMAIL is required for service account authentication",
				);
			}
