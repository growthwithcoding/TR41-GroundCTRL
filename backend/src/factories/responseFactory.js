...
const getMissionLingo = (httpStatus) => {
	switch (httpStatus) {
		...
		case 202:
			return {
				phrase: "GO",
				brief: "Command acknowledged. Processing orbital maneuver.",
			};
		case 204:
			return {
				phrase: "GO",
				brief: "Operation complete. No telemetry data to transmit.",
			};
		case 400:
			return {
				phrase: "NO-GO",
				brief: "Invalid trajectory parameters. Recalculate and retry.",
			};
		case 401:
			return {
				phrase: "NO-GO",
		...
		case 404:
			return {
				phrase: "NO-GO",
				brief: "Target not in tracking database. Verify coordinates.",
			};
		case 409:
			return {
				phrase: "NO-GO",
				brief: "Orbital slot conflict. Resource already exists in database.",
			};
		case 422:
			return {
				phrase: "NO-GO",
		...
		case 500:
			return {
				phrase: "ABORT",
				brief: "Critical system failure. Ground control investigating anomaly.",
			};
		case 501:
			return {
				phrase: "NO-GO",
				brief: "Flight plan drafted, systems not commissioned yet.",
			};
		case 502:
			return {
				phrase: "ABORT",
				brief: "Upstream relay failure. Ground station communication lost.",
			};
		case 503:
			return {
				phrase: "HOLD",
				brief: "Ground stations offline. Awaiting uplink restoration.",
			};
		case 504:
			return {
				phrase: "HOLD",
				brief: "Satellite response timeout. Link budget may be insufficient.",
			};
		default:
			if (httpStatus >= 200 && httpStatus < 300)
				return {
					phrase: "GO",
					brief: "All systems green. Mission parameters within tolerances.",
				};
			if (httpStatus >= 400 && httpStatus < 500)
				return {
					phrase: "NO-GO",
					brief: "Command rejected. Verify parameters and retry transmission.",
				};
			return {
				phrase: "ABORT",
				brief: "Unexpected anomaly detected. Mission Control investigating.",
			};
	}
};
...
function createErrorResponse(error, options = {}) {
	const { callSign = "SYSTEM", requestId = uuidv4() } = options;
